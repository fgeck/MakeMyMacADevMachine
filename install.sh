...
## CLI Tools, languages...
brew install golang python pyenv ffmpeg lame nmap openssl \
            tree vim wget jq yq fzf tldr tmux thefuck ranger mtr htop \
            pinentry-mac youtube-dl kubernetes-cli openjdk maven

## Dev tools
brew install --cask docker visual-studio-code postman hiddenbar stats rectangle \
    alt-tab slack discord itsycal vlc signal telegram firefox 
